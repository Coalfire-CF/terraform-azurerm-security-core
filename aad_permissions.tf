...
resource "azurerm_role_assignment" "assign_app_sub_user_access" {
  for_each             = { for enty in local.app_sub_user_mapping : "${substr(local.app_sub_user_mapping.user, -12, -1)}_${substr(local.app_sub_user_mapping.subscription_id, -12, -1)}" => entry }
  scope                = "/subscriptions/${each.value.subscription_id}"
  role_definition_name = "User Access Administrator"
  principal_id         = each.value.user
}
