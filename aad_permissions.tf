...
resource "azurerm_role_assignment" "assign_app_sub_user_access" {
  for_each = var.enable_aad_permissions[0] ? tomap({ local.app_sub_user_mapping : "${substr(local.app_sub_user_mapping.user, -12, -1)}_${substr(local.app_sub_user_mapping.subscription_id, -12, -1)}" }) : {}
